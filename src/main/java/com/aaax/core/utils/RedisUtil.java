...
public class RedisUtil {
    ...
    public <T> T get(String key, Class<T> returnClass) {
        ...
        return value == null ? null : JSONUtil.convertFromObject(value, returnClass);
    }

    public <T> T get(String key, TypeReference<T> valueType) {
        Object value = this.get(key);
        return value == null ? null : JSONUtil.convertFromObject(value, valueType);
    }

    /**
     * Return a typed cache hit, or invoke {@code loader} on miss / null / deserialize failure.
     * Loader is responsible for persisting if desired (e.g. {@link #set}).
     */
    public <T> T getOrLoad(String key, Class<T> type, Supplier<T> loader) {
        try {
            ...
            // corrupt cache / deserialize failure → reload
        }
        return loader.get();
    }
     ...
     * 普通缓存获取
     * @param key 键
     * @return object or BizException
     */
    public Object getOrElseThrow(String key) {
        if (key == null) {
            throw new BizException(SystemResponse.SYS9999);
        }
        Object object = redisTemplate.opsForValue().get(key);
        if (object == null) {
            throw new BizException(SystemResponse.SYS9902, "key ->".concat(key));
        }
        return object;
    }
     ...
     * get Key
     * @param key 键
     * @return object or BizException with parameter
     */
    public Object getOrElseThrow(String key, Response response) {
        if (key == null) {
            throw new BizException(SystemResponse.SYS9999);
        }
        Object object = redisTemplate.opsForValue().get(key);
    ...
    public long increment(String key, long delta) {
        if (delta < 0) {
            throw new RuntimeException("adding number must > 0");
        }
    ...
    public long decrement(String key, long delta) {
        if (delta < 0) {
            throw new RuntimeException("minus number must be > 0");
        }
        return redisTemplate.opsForValue().increment(key, -delta);
    }
    ...
    public Set<String> getListByWildCard(String key) {
