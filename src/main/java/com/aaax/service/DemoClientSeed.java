...
public class DemoClientSeed implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (registeredClientRepository.findByClientId("aaax-demo") == null) {
            registeredClientRepository.save(SecurityConfig.demoClient(passwordEncoder));
            log.info("Seeded OAuth client aaax-demo");
        }
        if (registeredClientRepository.findByClientId("aaax-spa") == null) {
            registeredClientRepository.save(SecurityConfig.spaPublicClient());
