...
class ExtPasswordAuthenticationProviderHappyPathTest {
    ...
    void authenticate_shouldMintTokens() {
        ...
        Authentication userAuth = Authentication.builder()
                ...
                .user(User.builder().id(7L).username("user@test.com").build())
                .build();
        when(authenticationService.findValidRecordsByDynamicIdentifier("user@test.com")).thenReturn(userAuth);
        when(authorizationService.findById("7")).thenReturn(null);

        Instant now = Instant.now();
        Jwt accessJwt = Jwt.withTokenValue("access-jwt")
                .header("alg", "RS256").issuedAt(now).expiresAt(now.plusSeconds(300)).claim("sub", "7").build();
