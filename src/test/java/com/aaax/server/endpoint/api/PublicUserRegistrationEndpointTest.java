...
class PublicUserRegistrationEndpointTest {
    ...
    @BeforeEach
    void setUp() {
        ReflectionTestUtils.setField(endpoint, "systemInvoker", "AAAX");
    }
    ...
    void register_shouldDelegate() {
        ...
        PendingVerifyUserResponseDto pending =
                PendingVerifyUserResponseDto.builder().username("u@test.com").build();
        ...
        assertEquals("AAAX", request.getSourceSystem());
    ...
    void register_checkOnly_shouldDelegate() {
        ...
        PendingVerifyUserResponseDto ok =
                PendingVerifyUserResponseDto.builder().username("free@test.com").build();
    ...
    void create_shouldCreateUser() {
        ...
        Result<GetUserResponseDto> result = endpoint.create(request, List.of(), "APP");

        assertEquals("u_1", result.getData().getId());
        verify(registerUserUseCase).execute_external(any());
    }
